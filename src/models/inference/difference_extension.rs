...
pub fn difference_extension(
    ...
    if exp1.stmt.copula == Copula::Similarity() || exp2.stmt.copula == Copula::Similarity() {
        return Err("Difference_extension not possible.");
    }
    ...
    if exp1.stmt.right == exp2.stmt.right {
        ...
        Err("Difference_extension not possible.")
    }
}
...
mod tests {
    ...
    fn test_difference_extension() {
        ...
        let result = difference_extension(&experience_base, 1, 2).unwrap();
        ...
        let result = difference_extension(&experience_base, 1, 3);
        assert_eq!(result, Err("Difference_extension not possible."));
    }
}
