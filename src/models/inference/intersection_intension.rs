...
pub fn intersection_intension(
    ...
    if exp1.stmt.copula == Copula::Similarity() || exp2.stmt.copula == Copula::Similarity() {
        return Err("Intersection_intension not possible.");
    }
    ...
    if exp1.stmt.left == exp2.stmt.left {
        ...
        Err("Intersection_intension not possible.")
    }
}
...
mod tests {
    ...
    #[test]
    fn test_intersection_intension() {
        ...
        let result = intersection_intension(&experience_base, 1, 2).unwrap();
        ...
        let result = intersection_intension(&experience_base, 1, 3);
        assert_eq!(result, Err("Intersection_intension not possible."));
    }
}
