...
pub fn resemblance(
    ...
    if exp1.stmt.copula != Copula::Similarity() || exp2.stmt.copula != Copula::Similarity() {
        return Err("Resemblance only possible with similarity.");
    }
    ...
    } else if exp1.stmt.right == exp2.stmt.left {
        Ok((
            Statement {
                left: exp2.stmt.right.clone(),
                copula: Copula::Similarity(),
    ...
    } else if exp1.stmt.right == exp2.stmt.right {
        Ok((
            Statement {
                left: exp2.stmt.left.clone(),
                copula: Copula::Similarity(),
...
mod tests {
    ...
    fn test_resemblance() {
        ...
        let result = super::resemblance(&experience_base, 1, 2);
        assert!(result.is_ok());
        let result = result.unwrap();
        assert_eq!(result.0, Statement::new("f <-> d").unwrap());
        assert_eq!(
            result.1.to_string(),
            TruthValue::new_from_str("<0.40, 0.71>")
                .unwrap()
                .to_string()
        );
    }
